...
public record RegionGuiUpdateS2CPacket(
    ...
    public static final Identifier ID = PrivitMod.id("region_gui_update_s2c");
    ...
    public static RegionGuiUpdateS2CPacket read(PacketByteBuf buf) {
        ...
        Optional<Boolean> isAreaLimitExceeded = buf.readBoolean() ? Optional.of(buf.readBoolean()) : Optional.empty();

        return new RegionGuiUpdateS2CPacket(
                regionId, isChanged, editorName, name, realBounds, draftBounds,
                conflictBounds, pivotPos, color, groups, isCreated, isAreaLimitExceeded
        );
    }
